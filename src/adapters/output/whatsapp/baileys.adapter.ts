...
export class BaileysAdapter implements WhatsappPort {
  ...
  this.sock.ev.on('messages.upsert', async (m: any) => {
    ...
    // ✅ FILTER: Jangan process pesan yang KITA kirim sendiri
    if (msg.key.fromMe) {
    ...
    console.log('[Baileys] ✅ Processing incoming message:', parsedMessage.id, 'text:', parsedMessage.text);
    ...
    this.sock.ev.on('message-receipt.update', (updates: any[]) => {
      console.log('[Baileys] 🔔 message-receipt.update event triggered');
      console.log('[Baileys] Raw updates:', JSON.stringify(updates, null, 2));

      for (const upd of updates) {
        console.log('[Baileys] Processing receipt:', {
          messageId: upd.key?.id,
          readTimestamp: upd.receipt?.readTimestamp,
          deliveryTimestamp: upd.receipt?.deliveryTimestamp,
          userReceipt: upd.userReceipt
        });

        if (upd.receipt?.readTimestamp) {
          console.log('[Baileys] ✅ READ receipt detected for:', upd.key.id);
          callback({
            messageId: upd.key.id!,
            status: 'read',
          });
        } else if (upd.receipt?.deliveryTimestamp) {
          console.log('[Baileys] ✅ DELIVERED receipt detected for:', upd.key.id);
          callback({
            messageId: upd.key.id!,
            status: 'delivered',
          });
        } else {
          console.log('[Baileys] ⚠️  No readTimestamp or deliveryTimestamp found');
        }
      }
    });
  }
  ...
  async sendMessage(chatId: string, content: { text?: string; quotedId?: string }): Promise<string> {
    ...
    try {
      ...
      // ✅ TRACK: Simpan ID pesan yang kita kirim, biar bisa skip nanti di onMessage
